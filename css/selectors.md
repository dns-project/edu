...
полне очевидно, что наиболее предпочтительным с точки зрения производительности является использование селекторов, требующих минимального количества проверок, причем проверок самых простых, в первую очередь под это описание подходят простые селекторы по элементу, по идентификатору и по классу, чуть больше занимает времени обработка по селектору *, еще больше ресурсов отнимет обработка CSS-правила с селектором по атрибуту, соответсвенно по значению атрибута будет отниаться ещё больше времени, также крайне «медленным» будет простой селектор, использующий динамический псевдокласс :hover (т.к. будет осуществляться проверка, не наведен ли на него курсор мышки), излишнее уточнение тоже вредно(нет надобности вместо #header писать div#header).
